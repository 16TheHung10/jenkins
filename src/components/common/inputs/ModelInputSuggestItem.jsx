...
export default function ModelInputSuggestItem({ ...props }) {
  ...
  const callApi = async (value) => {
    if (value?.length >= 3) {
      if (
        ...
        try {
          const response = await AuthApi.get(url, params, null, process.env.REACT_APP_EXT_ITEM_ROOT_URL);
  ...
  }, [dataSource, inputValue, selectedItem]);
