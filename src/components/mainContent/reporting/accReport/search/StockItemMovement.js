...
export default function StockItemMovement() {
  ...
  const handleExport = (arr) => {
    ...
    handleExportAutoField(dataExport, 'exportStockItemMovement', null, null, typeCol);
  };
  ...
  const handleSearch = async (currentPage, currentPageSize) => {
    ...
    try {
      ...
      const url = `/inventory/stockitem/movement`;
                  ...
                  <Row gutter={[16, 8]}>
                    ...
                    <Col xl={6} xxl={4}>
                      <label htmlFor="itemCode" className="w100pc">
                        Barcode - Item name:
                        <AutocompleteBarcode barCodes={allItem} updateFilter={(val, key) => setItemCode(val)} />
                      </label>
                    </Col>
                  </Row>
  ...
  const tableComp = useCallback(() => {
    ...
    // (obj.openQty + obj.receivingQty + obj.internalReceivingQty) -
    // (obj.saleQty + obj.disposalQty + obj.transferQty + obj.storeUsedQty + obj.returnQty + obj.lostQty)
        ...
        <Row gutter={16} className="mrt-10">
          <Col xl={24}>
            <TableCustomComp data={results} columns={columns} isLoading={isLoading}>
              {results?.length > 0 && (
                <div className="d-inline-block f-left">{`${results[0]?.itemCode} - ${results[0]?.itemName}`}</div>
              )}
            </TableCustomComp>
          </Col>
        </Row>
