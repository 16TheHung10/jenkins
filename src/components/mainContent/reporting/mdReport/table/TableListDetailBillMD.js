...
export default class TableListDetailBillMD extends BaseComponent {
  constructor(props) {
    super(props);
  ...
  // 	// if (newProps.objItems !== this.objItems) {
  // 	// 	this.objItems = newProps.objItems;
  // 	// }
  ...
  renderComp = () => {
    let items = Object.values(this.props.objItems).sort(
    ...
    return (
      <section id={this.idComponent}>
          ...
          <table className="table">
            <thead style={{ position: "sticky", top: 0 }}>
              <tr>
                ...
                <th className="fs-10 text-center">Discount</th>
                <th className="fs-10 text-center">
                  Gross <br />
                  sales
                </th>
                  ...
                  <Fragment key={objItems[item].invoiceCode}>
                    ...
                    >
                      ...
                      <td className="fs-10 rule-date">
                        {DateHelper.displayTime(objItems[item].invoiceDate)}
                      </td>
                      <td
                        className={
                          "fs-10 text-center " +
                          this.handleHighlight(objItems[item].payCustomer)
                        }
                      >
                        {StringHelper.formatValue(objItems[item].payCustomer)}
                      </td>
                      <td
                        className={
                          "fs-10 text-center " +
                          this.handleHighlight(objItems[item].returnPaid)
                        }
                      >
                        {StringHelper.formatValue(objItems[item].returnPaid)}
                      </td>
                      <td
                        className={
                          "fs-10 text-center " +
                          this.handleHighlight(objItems[item].totalAmount)
                        }
                      >
                        {StringHelper.formatValue(objItems[item].totalAmount)}
                      </td>
                      <td
                        className={
                          "fs-10 text-center " +
                          this.handleHighlight(objItems[item].discount)
                        }
                      >
                        {StringHelper.formatValue(objItems[item].discount)}
                      </td>
                      <td
                        className={
                          "fs-10 text-center " +
                          this.handleHighlight(objItems[item].billGrossSales)
                        }
                      >
                        {StringHelper.formatValue(
                          objItems[item].billGrossSales,
                        )}
                      </td>
                      ...
                      <td className="fs-10">
                        ...
                        {this.props.itemsLog &&
                        this.props.itemsLog.filter(
                          (el) => el.invoiceCode == objItems[item].invoiceCode,
                        ) &&
                        this.props.itemsLog.filter(
                          (el) => el.invoiceCode == objItems[item].invoiceCode,
                        ).length > 0 ? (
                          <span
                            className="badge bg-success text-dark fs-9"
                            style={{ padding: "3px 7px" }}
                            onClick={() => {
                              this.props.updateInvoiceSeleted(
                                objItems[item].invoiceCode,
                              );
                            }}
                          >
                            View log
                          ...
                          <>
                            {this.props.isShowLog === false ? null : (
                              <span
                                className="badge bg-warning text-dark fs-9"
                                style={{ padding: "3px 7px" }}
                              >
                                Updating
                              </span>
                    ...
                    {objItems[item].details &&
                      objItems[item].details.length > 0 && (
                        <tr
                            ...
                            >
                              <thead style={{ zIndex: 1 }}>
                                <tr>
                                  ...
                                  {objItems[item].listpayment &&
                                  objItems[item].listpayment.length > 0
                                    ? objItems[item].listpayment.map(
                                        (a, ia) => (
                              ...
                              <tbody>
                                <tr>
                                  ...
                                  {objItems[item] &&
                                    objItems[item].listpayment &&
                                    objItems[item].listpayment.map((a, ia) => (
                                      <td
                                        key={index + ia + item}
                                        rowSpan={
                                          objItems[item].details.length + 1
                                        }
                                        className={
                                          "text-center " +
        ...
        {/* <div className="wrap-table table-chart" style={{ maxHeight: 'auto', overflow: 'initial' }}>
					...
					<table className={"table table-hover " + (Object.keys(objItems).length > 0 && "mH-370")} style={{ maxHeight: 'calc(100vh - 271px)', overflow: 'auto', display: 'block' }}>
						<thead>
							<tr>
								<th>Invoice code</th>
								...
								Object.keys(objItems).map((item, index) => (
									<Fragment key={objItems[item].invoiceCode}>
										...
										<tr className="tb-parent" data-group="itemGroup" data-itemid={item}>
											<td>{objItems[item].invoiceCode}</td>
											<td className="rule-date">{DateHelper.displayTime(objItems[item].invoiceDate)}</td>
											<td className={"text-center " + this.handleHighlight(objItems[item].payCustomer)}>{StringHelper.formatValue(objItems[item].payCustomer)}</td>
											<td className={"text-center " + this.handleHighlight(objItems[item].returnPaid)}>{StringHelper.formatValue(objItems[item].returnPaid)}</td>
											<td className={"text-center " + this.handleHighlight(objItems[item].totalAmount)}>{StringHelper.formatValue(objItems[item].totalAmount)}</td>
											<td className={"text-center " + this.handleHighlight(objItems[item].discount)}>{StringHelper.formatValue(objItems[item].discount)}</td>
											<td className={"text-center " + this.handleHighlight(objItems[item].billGrossSales)}>{StringHelper.formatValue(objItems[item].billGrossSales)}</td>
											<td>{objItems[item].employeeCode}</td>
											<td>{objItems[item].employeeName}</td>
											<td><span className='btn-org text-center d-inline-block btn-expand-tr-items' style={{ width: 20, height: 20, borderRadius: 2, lineHeight: "16px" }} onClick={(e) => this.handleExpandItems(e)}>+</span></td>
										</tr>
															...
															<tr>
																...
																<td style={{ height: 0, padding: 0, border: 'none' }}></td>
																{
																	objItems[item] && objItems[item].listpayment && objItems[item].listpayment.map((a, ia) => (
																		<td key={index + ia + item} rowSpan={objItems[item].details.length + 1} className={"text-center " + this.handleHighlight(a.amount)}>{StringHelper.formatValue(a.amount)}</td>
																	))
																}
															</tr>
