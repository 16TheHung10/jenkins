...
import { useStoreOperationContext, useAppContext } from 'contexts';
...
const OperationStoreDetailsComp = ({ initialStoreCode }) => {
  ...
  const handleGetStoresData = async () => {
    ...
    const res = await storeModel.getListStore('store,suppliers');
    if (res.status) {
      const listStore = [...res.data?.stores];
      const store = listStore.find((el) => el.storeCode === initialStoreCode);
      return store;
    } else {
  ...
  const onChangeCityFC = (value) => {};
  const fieldInpus = () => {
    if (!isFranchiseStore) {
      return [
        ...FormField.StoreOPOriginal.fieldInputs({
          locationTypeOption,
          statusOption,
          cityOption,
          regionOption,
        ...
        ...FormField.StoreOPOriginal.fieldInputsFC({
          cityOptionFC,
    ...
    <section className="wrap-section">
      ...
      <div className="row header-detail">
        ...
        <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6">
          ...
          >
            {params.id ? 'Detail #' + params.id : 'Convert to ' + (store?.storeFC === '' ? 'franchise' : 'store')}
          </h3>
        </div>
...
export default OperationStoreDetailsComp;
