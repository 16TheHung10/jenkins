...
export default class ListSearchPromotionBuyOneGetOne extends BaseComponent {
  constructor(props) {
    super(props);
  ...
  renderComp = () => {
    ...
    return (
      <section id={this.idComponent}>
          ...
          <table className="table d-block w-full" style={{ maxHeight: 'calc(100vh - 230px)', overflow: 'auto' }}>
            ...
            <tbody>
              {items
                ...
                ?.map((item, i) => (
                  <tr key={i} onDoubleClick={() => this.handleToDetail(item.promotionCode)} className="cursor-pointer">
                    <td>
                      <input
        ...
        {/* {items.length !== 0 ? (
          <Paging page={this.page} itemCount={this.props.itemCount} pageSize2={15} onClickPaging={this.handleClickPaging} onClickSearch={this.handleLoadResult} listItemLength={this.items.length} />
        ) : null} */}
      </section>
    );
  };
