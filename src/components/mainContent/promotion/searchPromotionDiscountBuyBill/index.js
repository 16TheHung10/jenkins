...
import ListSearch from 'components/mainContent/promotion/listSearchPromotionDiscountBuyBill';
...
export default class SearchPromotionDiscountBuyBill extends BaseComponent {
  constructor(props) {
    super(props);
  ...
  handleLoadResult = () => {
    ...
    let page = '/promotion/bill/search';

    let params = {
      name: fields.promotionCode,
      startDate: fields.startDate !== '' ? DateHelper.displayDateFormat(fields.startDate, CONSTANT.FORMAT_DATE_PAYLOAD) : '',
      endDate: fields.endDate !== '' ? DateHelper.displayDateFormat(fields.endDate, CONSTANT.FORMAT_DATE_PAYLOAD) : '',
    ...
    model.getList(page, params).then((res) => {
      if (res.status && res.data) {
        if (res.data.promotions) {
          ...
          this.items = res.data.promotions.filter((el) => el.type === 2);
        }
      }
    ...
    //http://devapi.gs25.com.vn:8091/storemanagement/portal/promotion/{promotionType}/search?name=&fromDate=2018-01-01&toDate=2022-01-01&pageNumber=1&pageSize=30
  };
            ...
            <Row gutter={16}>
              <Col xl={6}>
                <label htmlFor="storeCode" className="w100pc">
                  Store:
                  <SelectBox data={storeOptions} func={this.handleUpdateStore} defaultValue={fields.storeCode} isMode={''} />
                </label>
              </Col>
              <Col xl={6}>
