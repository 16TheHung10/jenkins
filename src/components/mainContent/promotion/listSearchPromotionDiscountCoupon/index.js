...
export default class ListSearchPromotionDiscountCoupon extends BaseComponent {
  constructor(props) {
    super(props);
  ...
  handleToDetail = (codeId) => {
    this.targetLink('/promotion-discount-coupon/' + codeId);
  };
            ...
            <tbody>
              {items.map((item, i) => (
                <tr key={i} onDoubleClick={() => this.handleToDetail(item.promotionCode)}>
                  ...
                  <td className="fs-12">
                    <>
                      <p className="m-0 color-primary">{item.promotionName}</p>
                      <p className="m-0 hint">Code: {item.promotionCode}</p>
                    </>
                  </td>
                  <td className="fs-12">
