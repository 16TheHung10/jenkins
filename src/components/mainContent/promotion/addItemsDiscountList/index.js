...
class SearchItemsDiscountList extends BaseComponent {
  constructor(props) {
    super(props);
    this.items = [];
    this.idComponent =
  ...
  handleAddItems = () => {
    ...
    for (var i = 0; i < eles.length; i++) {
      var itemField = $(eles[i]).find("[name='itemOptionCheck']");
      if (itemField.is(":checked")) {
  ...
  handleChangeTypeItem = (index, e) => {
    ...
    // arr[index].qtyReceiving = 1;
    // arr[index].discountAmount = 0;
    this.items = arr;
...
export default SearchItemsDiscountList;
