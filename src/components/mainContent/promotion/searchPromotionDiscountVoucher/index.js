...
import ListSearch from 'components/mainContent/promotion/listSearchPromotionDiscountVoucher';
...
export default class SearchPromotionDiscountVoucher extends BaseComponent {
  constructor(props) {
    super(props);
    ...
    model.getList(page, params).then((res) => {
      if (res.status && res.data) {
        if (res.data.promotions) {
          ...
          this.items = res.data.promotions.filter((el) => el.type === 3);
        }
      }
      ...
      <div className="form-filter">
        <Row>
          <Col xl={24}>
            ...
            <Row>
              <Col className=" mt-10 flex gap-10">
                <button type="button" className="btn btn-success h-30" onClick={this.handleSearch}>
                  Search
                </button>
                {this.items && this.items.length > 0 ? (
                  <ExportPromotionData promotionType="billvoucher" params={fields} />
                ) : null}
              </Col>
            </Row>
          </Col>
