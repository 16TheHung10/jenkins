...
import ControlDetail from 'components/mainContent/promotion/controlPromotionDiscountVoucherDetail';

import { message } from 'antd';
import moment from 'moment';
import WarningNote from 'components/common/warningNote/WarningNote';

export default class SearchPromotionDiscountVoucherDetail extends BaseComponent {
  constructor(props) {
    super(props);
      ...
      await model.getPromotion('bill', this.orderCode).then((res) => {
        if (res.status && res.data) {
          if (res.data.promotion) {
            ...
            this.billPromotionType = this.infoPromotion.billPromotionType || '';
          ...
          // this.targetLink("/promotion-discount-bill");
  ...
  handleSave = () => {
    ...
    if (this.voucherValue === '') {
      message.error('Please enter voucher amount');
      return;
    }

    if (parseFloat(this.voucherValue) < 1000) {
      message.error('Please enter voucher amount more than 1000 VND');
      return;
    }

    if (this.invoiceApplyInfor === '') {
      message.error('Please enter apply to bill amount');
      return;
    }

    if (parseFloat(this.invoiceApplyInfor) < 10000) {
      message.error('Please enter apply to bill amount more than 10000 VND');
      return;
    }
    if (!this.docCode) {
      ...
      model.putPromotion('bill', this.orderCode, params).then((res) => {
        if (res.status && res.data) {
          if (res.data.promotionCode && res.data.promotionCode !== '') {
            this.targetLink('/promotion-discount-voucher/' + res.data.promotionCode, '/promotion-discount-voucher');
          }
  ...
  updateBillPromotionType = (value) => {
    this.billPromotionType = value;
    this.refresh();
  };
      ...
      <section className="wrap-section">
          ...
          <div className="row header-detail ">
            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12 flex items-center gap-10">
              <div className="flex justify-content-between items-center w-full">
                <div className="">
                  <button
                    onClick={() => super.back('/promotion-discount-voucher')}
                    type="button"
                ...
                <WarningNote>
                  <div className="cl-red bg-note">
                    ...
                    <br />
                    - Loại khuyến mãi tặng voucher giảm giá cho hóa đơn tiếp theo khi mua đủ giá trị của hóa đơn được
                    tặng. (VD: Mua bill 100K => KH được tặng voucher 10K => Áp dụng cho lần thanh toán tiếp theo với
                    bill 30K).
                    <br />
                    - Thời gian áp dụng phải sau ngày hiện tại.
                    <br />
                    - Giá trị voucher không được lớn hơn giá trị hóa đơn mua.
                    <br />- Dept. of I&T note: cập nhật thông tin đã đăng ký với cơ quan, tổ chức nhà nước. đơn mua.
                  </div>
                </WarningNote>
              </div>
            </div>
        ...
        {this.isAllowShowForm ? (
          <ControlDetail
            ...
            updateBillPromotionType={this.updateBillPromotionType}
            condition={this.condition}
            discountAmount={this.discountAmount}
            voucherValue={this.voucherValue}
            invoiceApplyInfor={this.invoiceApplyInfor}
            billPromotionType={this.billPromotionType}
