...
export default class ListDetail extends BaseComponent {
  constructor(props) {
    ...
    this.isUpdateForm = !this.isCopyType && (this.infoPromotion.poCode || '') !== '';
    ...
    return (
      <div className="context menu">
        <ul className="menu-options">
          <li className="menu-option" onClick={(e) => this.handleCheckSOH($(this.getCurrentTarget()).attr('data-itemid'))}>
            <i>
  ...
  handleShowHideItemByFilter = (item) => {
    if (this.props.filter !== undefined) {
      ...
      if (this.props.filter.supplier && item.supplierCode !== this.props.filter.supplier) {
        status = 0;
      } else if (this.props.filter.subClass && item.subCategory !== this.props.filter.subClass) {
        status = 0;
      } else if (this.props.filter.itemType && item.itemType !== this.props.filter.itemType) {
        status = 0;
      } else if (this.props.filter.keyword && StringHelper.searchLike(item.itemName, this.props.filter.keyword) === -1) {
        status = 0;
      }
      if (status === 0) {
        return 'hide';
  ...
  renderControlItems = () => {
    let isShowActionItems = this.isAllowUpdate && (!this.isUpdateForm || (!this.infoPromotion.canCel && !this.infoPromotion.approved));
    return (
      <div>
        <div className="floating-btn" style={{ paddingTop: 0 }}>
          <div className="row">
            <div className="col-xs-12 col-sm-12 col-md-12 col-lg-12">
              {isShowActionItems && (
                <button type="button" onClick={this.handleShowItemsSearch} className="btn btn-success btn-showpp">
                  1. ADD ITEMS
                </button>
              )}
            </div>
          ...
          <div className="row">
            <div className="col-xs-6 col-sm-6 col-md-6 col-lg-6">
              {isShowActionItems && this.items.length ? (
                <button type="button" onClick={this.handleDeleteItems} className="btn btn-danger">
                  Delete items
                </button>
              ) : (
                ''
                ...
                <tbody>
                  {el.map((target, indexTarget) => (
                    <tr key={indexTarget} data-group="itemGroup">
                      ...
                      <td className="rule-number">{StringHelper.formatPrice(target.discountAmount)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
