import { Col, Row, DatePicker, Tag, message, Checkbox, Button } from 'antd'
...
import ModelGroupStore from 'modelComponent/ModelGroupStoreSalesFix';

const { MonthPicker } = DatePicker;

export default function ModelExportDataMultipleAcc({ ...props }) {
    ...
    const [requestinvoice, setRequestinvoice] = useState(0);
    ...
    useEffect(() => {
        if (!!storesales) {
            ...
            if (Object.keys(storesales)?.length === 0) {
                ...
                listStoreOpt = Object.values(storesales)?.map(el => ({ value: el.storeCode, label: `${el.storeCode} - ${el.storeName}`, openedDate: el.openedDate }));
            }
    ...
    const onSearch = async () => {
        ...
        if (dateModel !== 'range' && groupDate.month() > moment().month() && groupDate.year() > moment().year()) {
        ...
        <div>
            ...
            >
                ...
                <Row gutter={[16, 16]}>
                    ...
                    <Col xl={24} >
                        <Checkbox onChange={(e) => handleOnChangeCheckbox(e)}>Request invoice</Checkbox>
                    </Col>
                </Row>
