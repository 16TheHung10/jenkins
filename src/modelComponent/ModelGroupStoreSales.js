...
export default function ModelGroupStoreSales({ ...props }) {
  ...
  useEffect(() => {
    if (!!storesales) {
      ...
      if (Object.keys(storesales)?.length === 0) {
        ...
        listStoreOpt = Object.values(storesales)
          ?.filter((el) => el.status === 0 || el.status === 2)
          ?.map((el) => ({
            value: el.storeCode,
            label: `${el.storeCode} - ${el.storeName}`,
            isFranchise: el.isFranchise,
  ...
  useEffect(() => {
    if (props?.groupStore !== undefined) {
      ...
      const isStoreFC = storeOption.filter(
        (option) => option.value === props?.groupStore && props?.groupStore !== '' && option.isFranchise == true
      );
      // const isStoreFC = storeOption.filter(option => (option.value === props?.groupStore && props?.groupStore !== '' && option.fcModel !== ''))
  ...
  const content = () => {
    return (
      <>
        {props?.isLabel !== undefined && props?.isLabel === false ? null : (
          <label htmlFor="storeCode" className="w100pc">
            ...
            {props?.isLabel !== undefined && props?.isLabel === false ? (
              ...
              <>
                {storesales && Object.keys(storesales)?.length !== 0 && Object.keys(storesales)?.length > 1 && (
                  <Radio.Group onChange={handleRadioChange} value={fcModel} disabled={props.disabledRadio ?? false}>
                    <Radio value="">Direct</Radio>
                    <Radio value="F">Franchise</Radio>
                    {/* <Radio value="All">All store</Radio> */}
                  </Radio.Group>
                )}
              </>
            )}
            {props.mode !== undefined && props.mode === 'multiple' ? (
              <Checkbox onChange={handleSelectAllChange} checked={selectAll}>
                All {fcModel == '' ? 'store direct' : 'store franchise'}
              </Checkbox>
            ) : null}
          </label>
        )}
